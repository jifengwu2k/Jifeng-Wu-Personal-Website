---
title: Personal Digital Governance Policy
date: 2026-04-05
updated: 2026-04-05
categories:
  - "Reflections"
tags:
  - "essay"
excerpt: The document explains how my digital artifacts and platforms should be organized, disclosed, and handled if I die or become incapacitated.
sticky: 4
---

I have uploaded **Personal Digital Governance Policy** to Zenodo:

- Zenodo record: https://zenodo.org/records/19431196
- DOI: https://doi.org/10.5281/zenodo.19431196

You never know which one will come first, tomorrow or the accident. For that reason, I wrote this document so that if I suddenly die or become incapacitated:

- trusted people can act promptly and coherently;
- my digital presence can remain legible rather than collapse into silence or confusion; and
- my ongoing work can be continued, concluded, or handed off.

This document serves both as a living governance policy and as a directive for death or incapacity. It has three tasks:

1. To enumerate and classify the digital artifacts I create and the digital platforms I use.
2. To match digital artifacts to digital platforms best suited to store or disseminate them.
3. To specify what trusted people should do if I die or become incapacitated.

The underlying philosophy is simple: **public-first**. I shall make all my digital artifacts public, or prepare them for public release.

## BibTeX

```bibtex
@misc{wu_2026_19431196,
  author       = {Wu, Jifeng},
  title        = {Personal Digital Governance Policy},
  month        = apr,
  year         = 2026,
  publisher    = {Zenodo},
  doi          = {10.5281/zenodo.19431196},
  url          = {https://doi.org/10.5281/zenodo.19431196},
}
```
